def add_user(forces_as_dict, to_sides, user_to_be_added):
    for side, users in forces_as_dict.items():
        if user_to_be_added in users:
            return forces_as_dict
    if to_sides not in forces_as_dict:
        forces_as_dict[to_sides] = [user_to_be_added]
    else:
        forces_as_dict[to_sides].append(user_to_be_added)
    return forces_as_dict


def change_side(forces_as_dict, to_sides, user_to_be_changed):
    for side, users in forces_as_dict.items():
        if user_to_be_changed in users:
            forces_as_dict[side].remove(user_to_be_changed)
            return add_user(forces_as_dict, to_sides, user_to_be_changed)
    return add_user(forces_as_dict, to_sides, user_to_be_changed)
